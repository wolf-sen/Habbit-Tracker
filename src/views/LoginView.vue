  ...
  try {
    ...
    if (response.data && response.data.length > 0) {
      ...
      if (user.password === inputPassword.value) {
        ...
        Cookies.set('user', user.id, { expires: 1, sameSite: 'Strict' });
...
<template>
  <div class="mx-4 my-32 flex flex-col">
    ...
    <Password toggleMask class="mb-6" id="password" placeholder="Password" :feedback="false" v-model="inputPassword" />
